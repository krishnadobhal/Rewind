 ...
 *
 *   REWIND_ENABLED=1 pnpm -F deep-research-agent start          one model
 *   REWIND_ENABLED=1 pnpm -F deep-research-agent start:multi    two models
 *
 * Nothing here knows about recording. `buildGraph` instruments the models and tools
 * it was handed; unrecorded that is a no-op and `recorder` is null, so
