...
test('a whole run is one request, not one per step', async () => {
  ...
  //
  // This records with no delay between steps, so nothing has drained by the time we
  // flush. A real agent waits seconds on the model between boundary crossings, far
  // longer than the 250ms drain window, and sends roughly one request per crossing.
  // What batches reliably there is the cassette+step pair from one record().
