/**
 * Every recorded run, newest first. The entry point to everything else.
 *
 * A row carries the three things that decide whether you want it: did it finish, how
 * big was it, and did every step resolve. The tier chips are why — a run with a miss
...
/** Miss first, then the rest — the interesting tier leads. */
const ORDER: Tier[] = ['miss', 'exact', 'recorded'];

/** Short local time; the full timestamp lives in the detail pane. */
...
export function RunList({
  ...
  /** The run being compared against, when one is held. */
  against: string | null;
      ...
      <ul className={styles.list} role="listbox" aria-label="Recorded runs">
        {runs.data.map((run) => (
          <li key={run.run_id} className={styles.item}>
            ...
            {/* Only offered once a run is open — comparing needs something to compare to. */}
